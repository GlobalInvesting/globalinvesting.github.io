...
(function () {
  ...
  // Reuses the SAME data _lastEvents already holds (client-side, computed
  // from the same live calendar-data/calendar.json this whole panel already
  // polls every 90s) — no new fetch, no new backend workflow, no AI. The
  // "why it matters" line reuses _calMethodologyFor() verbatim, the exact
  // same category-level text already shown in this file's own hover
  ...
  function _briefSelectEvents(events) {
    const { monday, sunday } = _briefWeekWindow();
    const inWeek = (events || []).filter(ev => {
      const localISO = toLocalDateISO(ev.dateISO, ev.timeUTC);
      if (!localISO) return false;
      const [y, mo, da] = localISO.split('-').map(Number);
      const d = new Date(y, mo - 1, da); // local constructor, matches formatDate()'s convention
  ...
  function _briefGroupByDay(events) {
    const groups = new Map(); // localISO -> events[]
    for (const ev of events) {
      const localISO = toLocalDateISO(ev.dateISO, ev.timeUTC);
      if (!groups.has(localISO)) groups.set(localISO, []);
      groups.get(localISO).push(ev);
    }
  ...
  function renderBriefing(containerId) {
    ...
    const selected = _briefSelectEvents(_lastEvents || []);
    if (!selected.length) {
      container.innerHTML =
        `<div style="padding:16px 12px;color:var(--text3);font-size:11px;">` +
        `No qualifying events found for this week yet — check back once the calendar populates.</div>`;
      return;
    }
    ...
    for (const [localISO, dayEvents] of grouped) {
      ...
      for (const ev of dayEvents) {
        const why = _calMethodologyFor(ev.title) ||
          'Included for its market impact this week — see forecast/previous for context.';
        const local = toLocalTime(ev.dateISO, ev.timeUTC);
        ...
        html +=
          `<div class="cal-brief-row">` +
            ...
            `</div>` +
            `<div class="cal-brief-fc">Forecast: <b>${_escAttr(String(fcst))}</b> &nbsp;·&nbsp; Previous: <b>${_escAttr(String(prev))}</b></div>` +
            `<div class="cal-brief-why"><b>Why it matters:</b> ${_escAttr(why)}</div>` +
          `</div>`;
      }
    }
    container.innerHTML = html;
  ...
  function ensureBriefingStyles() {
    if (document.getElementById('cal-brief-style')) return;
    const s = document.createElement('style');
    s.id = 'cal-brief-style';
    s.textContent = `
      .cal-brief-header{padding:8px 12px 6px;font-size:10px;color:var(--text3);text-transform:uppercase;letter-spacing:.04em;border-bottom:1px solid var(--border);}
      .cal-brief-day{padding:10px 12px 4px;font-size:11px;font-weight:600;color:var(--text2);}
      .cal-brief-row{padding:6px 12px 10px;border-bottom:1px solid var(--border);}
      .cal-brief-row:last-child{border-bottom:none;}
      .cal-brief-row-head{display:flex;align-items:center;gap:6px;margin-bottom:3px;}
      .cal-brief-dot-high,.cal-brief-dot-med{width:6px;height:6px;border-radius:50%;flex-shrink:0;}
      .cal-brief-dot-high{background:var(--down);}
      .cal-brief-dot-med{background:#ffa726;}
      .cal-brief-time{font-size:10px;color:var(--text3);flex-shrink:0;}
      .cal-brief-ccy{font-size:10px;font-weight:600;color:var(--text2);flex-shrink:0;}
      .cal-brief-title{font-size:12px;font-weight:600;color:#fff;}
      .cal-brief-fc{font-size:10px;color:var(--text3);margin-bottom:3px;}
      .cal-brief-fc b{color:var(--text2);font-weight:600;}
      .cal-brief-why{font-size:11px;color:var(--text2);line-height:1.4;}
      .cal-brief-why b{color:var(--text3);font-weight:600;}
    `;
    document.head.appendChild(s);
  }
