...
// ═══════════════════════════════════════════════════════════════════
// PAIR DETAIL PANEL v2 — full-width tabbed panel below chart
// Replaces the old floating #pd-popover with #pair-detail-panel
// Industry standard: Bloomberg/Eikon instrument detail pattern
// ═══════════════════════════════════════════════════════════════════

// Active tab state
let _pdpActiveTab = 'overview';

function openPairPopover(rowEl, tvSym) {
  openPairDetailPanel(tvSym);
}

function openPairDetailPanel(tvSym) {
  const panel = document.getElementById('pair-detail-panel');
  if (!panel) return;

  // Toggle if same pair clicked again
  if (panel.dataset.sym === tvSym && panel.style.display !== 'none') {
    closePairDetailPanel();
    return;
  }
  ...
  _pdpSwitchTab('overview');

  // Populate all content
  updatePairDetail(tvSym);
}

function closePairPopover() { closePairDetailPanel(); }
...
let _pdpTabsInited = false;
function _pdpInitTabs() {
  if (_pdpTabsInited) return;
  _pdpTabsInited = true;
  document.querySelectorAll('.pdp-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      const tab = btn.dataset.pdpTab;
      _pdpSwitchTab(tab);
      // If news/events tab not yet loaded, trigger load
      const sym = document.getElementById('pair-detail-panel')?.dataset.sym;
      if (sym) {
        if (tab === 'news')   _pdpLoadNews(sym);
        if (tab === 'events') _pdpLoadEvents(sym);
      }
    });
  });
}

function _pdpSwitchTab(tab) {
  _pdpActiveTab = tab;
  document.querySelectorAll('.pdp-tab').forEach(btn => {
    const active = btn.dataset.pdpTab === tab;
    btn.classList.toggle('pdp-tab--active', active);
    btn.setAttribute('aria-selected', active ? 'true' : 'false');
  });
  document.querySelectorAll('.pdp-tab-pane').forEach(pane => {
    pane.classList.toggle('pdp-tab-pane--active', pane.id === 'pdp-tab-' + tab);
  });
}
...
async function updatePairDetail(tvSym) {
  const mainPanel = document.getElementById('pair-detail-panel');
  if (!mainPanel) return;
  ...
  let pct1w = null;
  if (rt?.pct1w != null) pct1w = rt.pct1w;
  ...
  // ── Update header ──────────────────────────────────────────────────
  const symEl   = document.getElementById('pdp-sym');
  const priceEl = document.getElementById('pdp-price');
  const chgEl   = document.getElementById('pdp-chg');
  const rangeEl = document.getElementById('pdp-range');
  if (symEl)   symEl.textContent = label;
  if (priceEl) priceEl.textContent = price != null ? price.toFixed(dec) : '—';
  if (chgEl) {
    chgEl.textContent = fmtPct(pct1d);
    chgEl.className   = 'pdp-chg' + (pct1d == null ? '' : pct1d > 0 ? ' up' : pct1d < 0 ? ' dn' : '');
  }
  if (rangeEl) rangeEl.textContent = (sessH != null && sessL != null)
    ? 'H ' + sessH.toFixed(dec) + '  L ' + sessL.toFixed(dec) : '';

  // ── OVERVIEW TAB ──────────────────────────────────────────────────
  const ovPane = document.getElementById('pdp-tab-overview');
  if (ovPane) {
    const ivColorCls = atmIv == null ? '' : atmIv > 12 ? 'dn' : atmIv > 7 ? '' : 'up';
    const ivCrossNote = (meta?.cross || nzdProxy) && atmIv != null ? '<span style="font-size:8px;color:var(--text3);margin-left:2px;">~est</span>' : '';
    ovPane.innerHTML = `
      <div class="pdp-ov-grid">
        <div class="pdp-ov-section">Price &amp; Rate</div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">1D Change</div>
          <div class="pdp-ov-val ${cls(pct1d)}">${fmtPct(pct1d)}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">1W Change</div>
          <div class="pdp-ov-val ${cls(pct1w)}">${fmtPct(pct1w)}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">Sess High</div>
          <div class="pdp-ov-val">${sessH != null ? sessH.toFixed(dec) : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">Sess Low</div>
          <div class="pdp-ov-val">${sessL != null ? sessL.toFixed(dec) : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">Spread</div>
          <div class="pdp-ov-val">${spreadPips != null ? spreadPips.toFixed(1) + ' pip' : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">ADR</div>
          <div class="pdp-ov-val">${adr != null ? adr + ' pip' : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">${base || 'Base'} Rate</div>
          <div class="pdp-ov-val">${cbBase != null ? cbBase.toFixed(2) + '%' : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">${quote || 'Quote'} Rate</div>
          <div class="pdp-ov-val">${cbQuote != null ? cbQuote.toFixed(2) + '%' : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">Carry Diff</div>
          <div class="pdp-ov-val ${cls(carryDiff)}">${carryDiff != null ? (carryDiff >= 0 ? '+' : '') + carryDiff.toFixed(2) + '%' : '—'}</div>
        </div>

        <div class="pdp-ov-section">Volatility &amp; Options</div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">HV 30d</div>
          <div class="pdp-ov-val">${hv30 != null ? hv30.toFixed(1) + '%' : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">ATM IV${ivCrossNote}</div>
          <div class="pdp-ov-val ${ivColorCls}">${atmIv != null ? atmIv.toFixed(1) + '%' : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">IV − HV</div>
          <div class="pdp-ov-val ${atmIv != null && hv30 != null ? cls(atmIv - hv30) : ''}">${atmIv != null && hv30 != null ? (atmIv > hv30 ? '+' : '') + (atmIv - hv30).toFixed(1) + '%' : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">25d RR (${rrBase})</div>
          <div class="pdp-ov-val ${cls(rrVal)}">${rrVal != null ? (rrVal >= 0 ? '+' : '') + rrVal.toFixed(2) : '—'}</div>
        </div>

        <div class="pdp-ov-section">COT Positioning ${cotCcy ? '· ' + cotCcy : ''}</div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">LF Net</div>
          <div class="pdp-ov-val ${cls(cotNet)}">${fmtNet(cotNet)}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">LF WoW Δ</div>
          <div class="pdp-ov-val ${cls(cotWow)}">${cotWow != null ? (cotWow > 0 ? '+' : '') + Math.round(cotWow).toLocaleString() : '—'}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">AM Net</div>
          <div class="pdp-ov-val ${cls(cotAmNet)}">${fmtNet(cotAmNet)}</div>
        </div>
        <div class="pdp-ov-cell">
          <div class="pdp-ov-lbl">Net % OI</div>
          <div class="pdp-ov-val ${cls(cotPctOI)}">${cotPctOI != null ? (cotPctOI > 0 ? '+' : '') + cotPctOI.toFixed(1) + '%' : '—'}</div>
        </div>

        <div class="pdp-ov-section">Retail Sentiment · Myfxbook</div>
        <div class="pdp-ov-bar-wrap">
          <div class="pdp-ov-bar-lbl">Long / Short ratio</div>
          <div class="pdp-ov-bar"><div class="pdp-ov-bar-fill" style="width:${retBarL}%"></div></div>
          <div class="pdp-ov-bar-nums">${retL != null ? retL + '% Long' : '—'} / ${retS != null ? retS + '% Short' : '—'}</div>
        </div>
      </div>
      ${cotWeek ? '<div style="padding:4px 10px;font-size:9px;color:var(--text3);font-family:var(--font-mono);border-top:1px solid var(--border);">COT ' + cotWeek + (rrVal != null ? ' · Saxo RR 1M indicative' : '') + ' · Myfxbook</div>' : ''}
    `;
  }

  // ── COT TAB ───────────────────────────────────────────────────────
  const cotPane = document.getElementById('pdp-tab-cot');
  if (cotPane) {
    const renderCotBlock = (ccy, net, wow, amNet, pctOI, oi, prevOI, totalOI) => {
      const oiDelta = (prevOI != null && oi != null) ? oi - prevOI : null;
      const oiDeltaStr = oiDelta == null ? '' : ` (${oiDelta > 0 ? '+' : ''}${Math.round(oiDelta).toLocaleString()})`;
      return `
        <div class="pdp-cot-block">
          <div class="pdp-cot-block-head">CFTC Disaggregated TFF · ${ccy} · Leveraged Funds (Futures + Options)</div>
          <div class="pdp-cot-grid">
            <div class="pdp-cot-cell">
              <div class="pdp-cot-lbl">LF Net Contracts</div>
              <div class="pdp-cot-val ${cls(net)}">${fmtNet(net)}</div>
            </div>
            <div class="pdp-cot-cell">
              <div class="pdp-cot-lbl">WoW Change</div>
              <div class="pdp-cot-val ${cls(wow)}">${wow != null ? (wow > 0 ? '+' : '') + Math.round(wow).toLocaleString() : '—'}</div>
            </div>
            <div class="pdp-cot-cell">
              <div class="pdp-cot-lbl">Asset Managers Net</div>
              <div class="pdp-cot-val ${cls(amNet)}">${fmtNet(amNet)}</div>
            </div>
            <div class="pdp-cot-cell">
              <div class="pdp-cot-lbl">Net % of LF OI</div>
              <div class="pdp-cot-val ${cls(pctOI)}">${pctOI != null ? (pctOI > 0 ? '+' : '') + pctOI.toFixed(1) + '%' : '—'}</div>
            </div>
            ${oi != null ? `
            <div class="pdp-cot-cell">
              <div class="pdp-cot-lbl">LF Open Interest</div>
              <div class="pdp-cot-val">${Math.round(oi).toLocaleString()}${oiDeltaStr ? '<span style="font-size:9px;color:var(--text3);"> ' + oiDeltaStr + '</span>' : ''}</div>
            </div>` : ''}
            ${totalOI != null ? `
            <div class="pdp-cot-cell">
              <div class="pdp-cot-lbl">Total Market OI</div>
              <div class="pdp-cot-val">${Math.round(totalOI).toLocaleString()}</div>
            </div>` : ''}
          </div>
        </div>`;
    };
    ...
    let cotContent = '';
    if (cotCcy && (cotNet != null || cotAmNet != null))
      cotContent += renderCotBlock(cotCcy, cotNet, cotWow, cotAmNet, cotPctOI, cotOI, cotPrevOI, cotTotalOI);
    if (cotCcy2 && (cot2Net != null || cot2AmNet != null))
      cotContent += renderCotBlock(cotCcy2, cot2Net, cot2Wow, cot2AmNet, cot2PctOI, cot2OI, null, null);
    if (!cotContent)
      cotContent = '<div class="pdp-evt-empty">No COT data available for this pair.</div>';

    cotPane.innerHTML = `
      <div class="pdp-cot-wrap">${cotContent}</div>
      ${cotWeek ? '<div class="pdp-cot-footer">CFTC · week ending ' + cotWeek + ' · Disaggregated TFF report · Leveraged Funds category · Options+Futures combined</div>' : ''}
    `;
  }

  // ── VOLATILITY TAB ────────────────────────────────────────────────
  const volPane = document.getElementById('pdp-tab-volatility');
  if (volPane) {
    const ivSrc = meta?.cross ? 'Triangulated from CBOE/CME component IVs' : nzdProxy ? 'AUD/USD CBOE vol × 1.08 proxy' : 'CBOE/CME FX Vol Index';
    const ivColorCls = atmIv == null ? '' : atmIv > 12 ? 'dn' : atmIv > 7 ? '' : 'up';
    const rrDir = rrVal == null ? '—' : rrVal > 0 ? 'Call skew (' + rrBase + ' upside bid)' : rrVal < 0 ? 'Put skew (downside protection bid)' : 'Neutral skew';
    volPane.innerHTML = `
      <div class="pdp-vol-grid">
        <div class="pdp-vol-section">Realised Volatility</div>
        <div class="pdp-vol-cell">
          <div class="pdp-vol-lbl">HV 30d (annualised)</div>
          <div class="pdp-vol-val">${hv30 != null ? hv30.toFixed(1) + '%' : '—'}</div>
          <div class="pdp-vol-sub">30 trading days · log returns · √252</div>
        </div>
        <div class="pdp-vol-cell">
          <div class="pdp-vol-lbl">ADR (pips)</div>
          <div class="pdp-vol-val">${adr != null ? adr : '—'}</div>
          <div class="pdp-vol-sub">Avg daily range · from HV30</div>
        </div>

        <div class="pdp-vol-section">Implied Volatility</div>
        <div class="pdp-vol-cell">
          <div class="pdp-vol-lbl">ATM IV</div>
          <div class="pdp-vol-val ${ivColorCls}">${atmIv != null ? atmIv.toFixed(1) + '%' : '—'}</div>
          <div class="pdp-vol-sub">${ivSrc}</div>
        </div>
        <div class="pdp-vol-cell">
          <div class="pdp-vol-lbl">IV − HV Premium</div>
          <div class="pdp-vol-val ${atmIv != null && hv30 != null ? cls(atmIv - hv30) : ''}">${atmIv != null && hv30 != null ? (atmIv > hv30 ? '+' : '') + (atmIv - hv30).toFixed(1) + '%' : '—'}</div>
          <div class="pdp-vol-sub">Options vs realised · risk premium</div>
        </div>

        <div class="pdp-vol-section">Options Skew · Saxo Bank 1M tenor · indicative</div>
        <div class="pdp-vol-cell">
          <div class="pdp-vol-lbl">25d Risk Reversal</div>
          <div class="pdp-vol-val ${cls(rrVal)}">${rrVal != null ? (rrVal >= 0 ? '+' : '') + rrVal.toFixed(2) : '—'}</div>
          <div class="pdp-vol-sub">${rrDir}</div>
        </div>
        <div class="pdp-vol-cell">
          <div class="pdp-vol-lbl">Skew interpretation</div>
          <div class="pdp-vol-val" style="font-size:10px;font-weight:500;">${rrVal == null ? '—' : Math.abs(rrVal) < 0.5 ? 'Balanced market' : Math.abs(rrVal) < 1.5 ? 'Mild directional bias' : 'Strong directional bias'}</div>
          <div class="pdp-vol-sub">25d call IV − 25d put IV</div>
        </div>
        <div class="pdp-vol-note">
          IV color: green ≤ 7% (low hedging cost) · orange 7–12% (elevated) · red > 12% (high hedging cost).
          25d RR positive = calls bid over puts (market pricing ${rrBase} upside); negative = puts bid (downside dominant).
          Source: CBOE/CME FX Vol Index for direct pairs; triangulation formula for crosses. Saxo Bank public options page for RR.
        </div>
      </div>
    `;
  }

  // ── NEWS tab: trigger load if currently active ─────────────────────
  if (_pdpActiveTab === 'news') _pdpLoadNews(tvSym);
  // ── EVENTS tab: trigger load if currently active ───────────────────
  if (_pdpActiveTab === 'events') _pdpLoadEvents(tvSym);
}

// ── News tab loader ─────────────────────────────────────────────────
let _pdpNewsCache = null;
async function _pdpLoadNews(tvSym) {
  const pane = document.getElementById('pdp-tab-news');
  if (!pane) return;
  ...
  const quote = meta?.quote || null;
  const label = meta?.label || tvSym.replace(/^.*:/,'').replace(/(.{3})(.{3})/,'$1/$2').toUpperCase();
  const currencies = [base, quote].filter(Boolean);

  // Show loading state if pane is empty
  if (!pane.dataset.loaded) {
    pane.innerHTML = '<div class="pdp-news-empty">Loading news…</div>';
  }

  // Fetch or use cache
  if (!_pdpNewsCache) {
    try {
      const r = await fetch('./news-data/news.json');
      if (r.ok) _pdpNewsCache = await r.json();
    } catch {}
  }

  if (!_pdpNewsCache?.articles) {
    pane.innerHTML = '<div class="pdp-news-empty">News feed unavailable.</div>';
    return;
  }

  pane.dataset.loaded = '1';

  // State: which currency filter is active ('all', base, quote)
  let activeFilter = 'all';

  const renderNews = (filter) => {
    activeFilter = filter;
    const articles = _pdpNewsCache.articles.filter(a => {
      if (filter === 'all') return currencies.includes(a.cur);
      return a.cur === filter;
    });

    const newsHtml = articles.length === 0
      ? '<div class="pdp-news-empty">No recent news for this pair.</div>'
      : articles.map(a => `
          <div class="pdp-news-item" data-id="${a.id}" tabindex="0" role="button" aria-expanded="false" onclick="this.classList.toggle('pdp-news-open');this.setAttribute('aria-expanded',this.classList.contains('pdp-news-open'))">
            <div class="pdp-news-imp ${a.impact || 'low'}"></div>
            <div class="pdp-news-body">
              <div class="pdp-news-title" title="${a.title.replace(/"/g,'&quot;')}">${a.title}</div>
              <div class="pdp-news-meta">
                <span class="pdp-news-cur">${a.cur || ''}</span>
                <span>${a.date || ''} ${a.time || ''}</span>
                <span>${a.source || ''}</span>
              </div>
              ${a.expand ? `<div class="pdp-news-expand">${a.expand}</div>` : ''}
            </div>
          </div>`).join('');

    const filterBtns = ['all', ...currencies].map(f => {
      const isActive = f === activeFilter;
      const lbl = f === 'all' ? 'All currencies' : f;
      return `<button class="pdp-news-filter-btn${isActive ? ' active' : ''}" onclick="_pdpNewsFilter('${f}')">${lbl}</button>`;
    }).join('');

    pane.innerHTML = `
      <div class="pdp-news-filter">
        ${filterBtns}
        <span class="pdp-news-source">news-data/news.json · ${_pdpNewsCache.updated_label || ''}</span>
      </div>
      <div class="pdp-news-list">${newsHtml}</div>
    `;
  };

  // Expose filter handler globally
  window._pdpNewsFilter = (f) => renderNews(f);

  renderNews(activeFilter);
}

// ── Events tab loader ────────────────────────────────────────────────
let _pdpCalCache = null;
async function _pdpLoadEvents(tvSym) {
  const pane = document.getElementById('pdp-tab-events');
  if (!pane) return;
  ...
  if (!pane.dataset.loaded) {
    pane.innerHTML = '<div class="pdp-evt-empty">Loading calendar…</div>';
  }

  if (!_pdpCalCache) {
    try {
      const r = await fetch('./calendar-data/ff_calendar.json');
      if (r.ok) _pdpCalCache = await r.json();
    } catch {}
  }

  if (!_pdpCalCache?.events) {
    pane.innerHTML = '<div class="pdp-evt-empty">Calendar unavailable.</div>';
    return;
  }
  ...
  // Get upcoming + recent events for both currencies
  const events = _pdpCalCache.events.filter(e =>
    currencies.includes(e.currency) && e.dateISO >= today
  ).sort((a, b) => {
    const da = a.dateISO + 'T' + (a.timeUTC || '99:99');
    const db = b.dateISO + 'T' + (b.timeUTC || '99:99');
    return da < db ? -1 : da > db ? 1 : 0;
  }).slice(0, 40);

  // Also get recent released events
  const released = _pdpCalCache.events.filter(e =>
    currencies.includes(e.currency) && e.dateISO < today && e.released
  ).sort((a, b) => b.dateISO < a.dateISO ? -1 : 1).slice(0, 20);

  const impDot = i => `<span class="pdp-evt-imp-dot ${i || 'low'}"></span>`;

  const actCls = (e) => {
    if (!e.actual || !e.forecast) return '';
    const act = parseFloat(e.actual), fore = parseFloat(e.forecast);
    if (isNaN(act) || isNaN(fore)) return '';
    // Higher = better heuristic: if forecast > previous, higher act = beat
    return act > fore ? 'beat' : act < fore ? 'miss' : '';
  };

  const renderRows = (evts) => evts.map(e => `
    <div class="pdp-evt-row">
      <span class="pdp-evt-time">${e.dateISO.slice(5)} ${e.timeUTC ? e.timeUTC.slice(0,5) : ''}</span>
      <span>${impDot(e.impact)}</span>
      <span class="pdp-evt-title" title="${(e.title || '').replace(/"/g,'&quot;')} · ${e.currency}">${e.currency} ${e.title || ''}</span>
      <span class="pdp-evt-prev">${e.previous ?? '—'}</span>
      <span class="pdp-evt-fore">${e.forecast ?? '—'}</span>
      <span class="pdp-evt-act ${actCls(e)}">${e.actual ?? (e.released ? '—' : '')}</span>
    </div>`).join('');

  pane.innerHTML = `
    <div class="pdp-evt-list">
      ${events.length ? `
        <div class="pdp-evt-head">Upcoming events · ${currencies.join(' &amp; ')}</div>
        <div class="pdp-evt-col-head">
          <span>Date / Time</span><span></span><span>Event</span>
          <span style="text-align:right">Prev</span>
          <span style="text-align:right">Fore</span>
          <span style="text-align:right">Act</span>
        </div>
        ${renderRows(events)}
      ` : '<div class="pdp-evt-empty">No upcoming events this week.</div>'}
      ${released.length ? `
        <div class="pdp-evt-head">Recent releases</div>
        <div class="pdp-evt-col-head">
          <span>Date / Time</span><span></span><span>Event</span>
          <span style="text-align:right">Prev</span>
          <span style="text-align:right">Fore</span>
    ...
    <div class="pdp-cot-footer">Source: Finnhub economic calendar · ${_pdpCalCache.generated_at ? _pdpCalCache.generated_at.slice(0,16).replace('T',' ') + ' UTC' : ''}</div>
  `;
}
