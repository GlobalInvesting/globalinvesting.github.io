    ...
    <div class="pd-inline">
      ...
      <div class="pd-inline-group">
        ...
        <div class="pd-inline-metrics">
          ...
          <div class="pd-inline-metric fx-tip" data-tip-title="ATM Implied Volatility" data-tip-body="${atmIvSource ? `ATM IV resolved via ${atmIvSource}.` : 'ATM IV from an institutional-grade source.'} Cascade tried in order until one resolves: CBOE/CME FX Volatility Index → PHLX World Currency Options → Saxo Bank cache → CME futures options/ETF → Barchart cache — same variance-swap methodology as VIX where CBOE/CME applies. ${atmIvRank != null ? 'Color = IV Rank vs 52-week range: green rank&lt;30 (historically cheap), red rank&gt;70 (historically expensive).' : 'Color = cost of hedging: green ≤7% (cheap), red >12% (expensive) — fixed band; IV Rank not yet available (needs ≥4 weeks history).'} Not a directional signal.">
  ...
  try {
    ...
    if (tbody) tbody.innerHTML = pairs.map(p => {
      ...
      if (etfIv?.iv != null) {
        ...
        const td1Body  = `ATM IV ${ivStr} from ${etfIv.source || 'CBOE/CME'} — institutional-grade source for OTC interbank implied vol. Green ≤7% (cheap vol); red >12% (expensive).`;
        const td1Ex    = 'EUR/GBP/JPY use the CBOE/CME FX Volatility Indexes (^EUVIX/^BPVIX/^JYVIX) — same variance-swap replication as VIX. AUD/NZD use the PHLX World Currency Options index (^XDA/^XDZ). CHF and CAD use CME futures options (6S/6C) with ETF fallback (FXF/FXC). All values have ~15min delay.';
