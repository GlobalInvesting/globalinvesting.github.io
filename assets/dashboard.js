...
async function updatePairDetail(tvSym) {
  ...
  panel.innerHTML = `
    ...
    <div class="pd-grid">
      ...
      <div class="pd-cell" data-tip="${meta?.cross && atmIv != null ? 'Synthesised from component USD-pair ETF option IVs via triangulation (√(IVa²+IVb²−2ρ·IVa·IVb)). Not OTC interbank — indicative only.' : '30-day ATM implied vol from CBOE FX ETF option chain (yfinance)'}"><div class="pd-lbl">ATM IV${meta?.cross && atmIv != null ? '<span style=\'font-size:8px;color:var(--text3);margin-left:2px;\'>~</span>' : ''}</div><div class="pd-val ${atmIv != null ? (atmIv > 12 ? 'down' : atmIv > 7 ? '' : 'up') : ''}">${atmIv != null ? atmIv.toFixed(1)+'%' : '—'}</div></div>
