 ...
 *   not a risk pattern; no eval/document.write/Function(); the one innerHTML
 *   sink is static SVG markup, no external data). Completed the v8.278.0
 *   internal-procedure-narration cleanup for this file — rewrote ~13
 *   instances across the version history, including two direct client
 *   quotes (one in English, one in Spanish), to state only the technical
 *   symptom/fix.
 *
 * v1.7.5 (2026-08-14): Found live — the free Overview
 ...
 *
 * v1.7.4 (2026-08-13): Found live — on mobile the activation modal
 ...
 *   unchanged.
 *
 * v1.7.3 (2026-08-12): Reverted to the prior link target — the
 ...
 *   untouched here) — not in scope for this change, flagged as a follow-up if the
 *   same reversion should apply there too.
 *
 * v1.7.2 (2026-08-12): Follow-up to v1.7.1's incomplete broker-access
 *   scroll-target fix. v1.4.0's original fix already documented
 ...
 *
 * v1.7.1 (2026-08-12): GUIDELINES.md compliance fix found live —
 ...
 *
 * v1.7.0 (2026-08-12): Serious bug found live the same day v1.6.0
 *   shipped its close button: closing the modal without activating left the
 *   full terminal open and functional, which must never happen. Root cause: the "Open full
 ...
 *   model, intentional for OTHER entry paths but not for this one). As long as the modal had no way
 ...
 *   Fix: hideModal() now returns to the free Overview snapshot by calling
 *   window.giShowOverview() (new in
 ...
 *   Found live against the new Overview snapshot (v8.129.0): a
 ...
 *   chosen over showing nothing until activation. The
...
/* v1.7.4: mobile — the modal previously only ever centered #gi-auth-box
   ...
   Found live on mobile: the modal filled the whole screen with the close
   button unreachable. Fix: below 640px, switch the backdrop from
