          ...
          if (container) {
            ...
            container.innerHTML = signals.map(s => {
              ...
              if (titleParts && footerParts) {
                // Priority is a single colored dot (Forex Factory calendar
                // convention: red/orange/yellow impact) — no text, no fill
                // shape, just color + size against the dark panel. Not fully
                // color-independent on its own, so aria-label/title carry the
                // High/Medium/Low text for screen readers and hover. Time
              ...
              // Fallback — unchanged from production, for signals without the
              // Rule 10/14 shape (e.g. legacy fetch_intraday_quotes.py entries).
