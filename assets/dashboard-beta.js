...
async function renderFairValue() {
  ...
  let html = '';
  // BETA: collected alongside the existing table-row loop, not a second
  // pass over the data — one entry per pair that actually has a real
  // regression fit this render (mirrors the table's own gating exactly,
  // so the chart can never show a pair the table itself couldn't).
  const zEntries = [];
  results.forEach(({ pair, rows }) => {
    ...
    if (usableForPair.length >= FV_MIN_ROWS) {
      ...
      if (reg && reg.residStd > 0 && lastHasAllFeatures) {
        ...
        // BETA: same z value already computed above for the table cell —
        // reused, not recomputed, so the chart and table can never disagree.
        zEntries.push({
