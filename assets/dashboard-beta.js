...
/* ===================== GI Alert System (beta) =====================
   Three-layer in-terminal alerting, matching the institutional-terminal
   convention (in-app toast + sound cue + persistent history tray),
   layered alongside the existing native OS Notification calls below —
   it does not replace them, since a backgrounded/minimized tab still
   needs the OS layer. */
...
function giInitAlertSystem() {
  ...
  if (soundBtn) {
    soundBtn.addEventListener('click', () => {
      /* This click is also the required user gesture that unlocks Web Audio
         autoplay in every major browser — resolving/creating the context
         here, on the same gesture, means the very first real alert's chime
         is never silently dropped by the browser's autoplay policy. */
      if (!_giAudioCtx) {
