...
/* ==== Smooth panel transitions (beta) ====================================
 * Single shared primitive for "click a control, panel content changes"
 * interactions (tab/toggle switches, modal/fullscreen opens). One
 * implementation, reused everywhere, instead of each call site inventing
 * its own timing/easing — avoids the dual-implementation-drift class of
 * bug this project's own GUIDELINES already flags for duplicated logic.
 * Respects prefers-reduced-motion (bypassed entirely, no animation).
 * Scoped to the specific container being swapped — never a whole-page
 * cross-fade — so unrelated parts of the screen never visibly flicker.
 */
const GI_TRANSITION_MS = 160;
const GI_TRANSITION_EASE = 'cubic-bezier(.4,0,.2,1)';

function _giReducedMotion() {
  return typeof window.matchMedia === 'function'
    && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
}

// Fades `el` out, runs `updateFn` (the actual DOM mutation — innerHTML swap,
// class toggles, etc.), then fades the new content in. `updateFn` must be
// synchronous; for content that arrives later via fetch, call this around
// the synchronous part only (the loading state fades in immediately, same
// as today, just smoothly instead of as a hard cut).
// For panels that swap by toggling display (show one, hide its siblings)
// rather than an innerHTML replacement — the old pane disappears instantly
// (imperceptible, since it's simultaneously covered by the new one), the
// new pane fades/slides in. Call this right after the pane becomes visible.
function _giFadeIn(el) {
  if (!el || _giReducedMotion()) return;
  el.style.transition = 'none';
  el.style.opacity = '0';
  el.style.transform = 'translateY(2px)';
  void el.offsetHeight;
  el.style.transition = `opacity ${GI_TRANSITION_MS}ms ${GI_TRANSITION_EASE}, transform ${GI_TRANSITION_MS}ms ${GI_TRANSITION_EASE}`;
  el.style.opacity = '1';
  el.style.transform = 'translateY(0)';
}

function _giSmoothSwap(el, updateFn) {
  if (!el || _giReducedMotion()) { updateFn(); return; }
  el.style.transition = `opacity ${GI_TRANSITION_MS}ms ${GI_TRANSITION_EASE}, transform ${GI_TRANSITION_MS}ms ${GI_TRANSITION_EASE}`;
  el.style.opacity = '0';
  el.style.transform = 'translateY(2px)';
  window.setTimeout(() => {
    updateFn();
    void el.offsetHeight; // force reflow so the fade-in starts from opacity:0
    el.style.opacity = '1';
    el.style.transform = 'translateY(0)';
  }, GI_TRANSITION_MS);
}
...
function initCOTAssetTabs() {
  ...
  tabBar.addEventListener('click', e => {
    ...
    const cotRowsEl = document.getElementById('cot-rows');
    _giSmoothSwap(cotRowsEl, () => {
      if (asset === 'fx') {
        const subEl = document.getElementById('cot-date-sub');
...
function initSentimentAssetTabs() {
  ...
  tabBar.addEventListener('click', e => {
    ...
    _giSmoothSwap(document.getElementById('sent-rows'), () => _renderSentimentForActiveTab());
  });
}
...
function _initCarryRankToggle() {
  ...
  btnCarry.addEventListener('click', () => {
    _carryRankMode = 'carry';
    _giSmoothSwap(document.getElementById('carry-rank-rows'), () => _renderCarryRankRows('carry'));
  });
  btnCV.addEventListener('click', () => {
    _carryRankMode = 'carryVol';
    _giSmoothSwap(document.getElementById('carry-rank-rows'), () => _renderCarryRankRows('carryVol'));
  });
}
...
function initG8RatesTabs() {
  ...
  tabBar.addEventListener('click', e => {
    ...
    if (pane) { pane.style.display = ''; _giFadeIn(pane); }
...
function _intelFsSetTab(tab) {
  ...
  document.querySelectorAll('.intel-group').forEach(function (grp) {
    const active = grp.id === 'intel-group-' + tab;
    grp.classList.toggle('intel-group-active', active);
    if (active) _giFadeIn(grp);
  });
}
