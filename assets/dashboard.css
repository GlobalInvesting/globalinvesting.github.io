*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ticker-label {
    ...
    font-family: var(--font-ui);
    color: var(--blue); text-transform: uppercase; letter-spacing: .10em;
    border-right: 1px solid var(--border2); white-space: nowrap; flex-shrink: 0;
  }
  .ticker-track {
    display: flex; align-items: center;
    white-space: nowrap; /* animation injected dynamically by buildNewsTicker() */
  }
  ...
  /* @keyframes ticker-scroll removed — replaced by JS-injected ticker-exact keyframe */
  ...
  /* When rightpanel is hidden, collapse its grid column */
  ...
  /* Watchlist is the last section — grows to fill leftover sidebar height, no blank gap */
  #watchlist-section {
    flex: 1 0 auto;
    border-bottom: none;
  }
  ...
  /* Crosses scrollable list — match sidebar scrollbar style */
  ...
  /* ── Sidebar expand: vertical layout override ──
     pd-inline is designed for horizontal scroll in the wide FX table.
     Inside the 180px sidebar we restack all groups vertically. */
  .sb-expand-inner .pd-inline-scroll { overflow-x: hidden; }
  .sb-expand-inner .pd-inline { flex-direction: column; width: 100%; }
  /* Hide the price block — symbol, rate and chg are already visible in the sb-row above */
  .sb-expand-inner .pd-inline-price { display: none; }
  /* Each group becomes a full-width row */
  ...
  /* Metrics wrap tightly in the narrow column */
  ...
  /* Suppress TradingView copyright bar that causes gap */
  ...
  /* Kill any bottom spacing inside TV chart container */
  ...
  /* pointer-events:none on wrap kills the 32px margin-bottom overlap intercept;
     restored on iframe so chart pan/zoom/interaction still works */
  ...
  /* Key Correlations — compact layout to fit 12 rows without clipping */
  ...
  /* Signal notification toggle — in Alerts & Market Signals panel-head */
  .sig-notif-toggle {
  ...
  .sig-notif-toggle:hover { color: var(--blue); border-color: var(--blue); }
  ...
  .narr-regime {
    ...
    white-space: nowrap; align-self: center; letter-spacing: .04em;
    font-family: var(--font-ui);
  }
  .narr-regime.stale {
  ...
  /* ── Narrative share ghost button — visible on #narrative hover only ── */
  .narr-share-btn {
  ...
  .cot-header {
    display: grid;
    grid-template-columns: 36px 1fr 42px 64px 52px 14px 46px 68px;
  ...
  .cot-sym { font-weight: 700; color: #fff; font-family: var(--font-ui); }
  .cot-bar-outer { height: 10px; background: var(--bg3); position: relative; overflow: hidden; }
  ...
  /* WoW Δ — primary momentum signal */
  .cot-wow { text-align: right; font-size: 10px; font-family: var(--font-mono); }
  /* LF/AM divergence dot — filled circle = aligned, hollow = diverging */
  ...
  /* SESSIONS */
  .session-row { display: flex; gap: 4px; padding: 6px 10px; border-bottom: 1px solid var(--border); }
  .sess { padding: 2px 8px; font-size: 10px; font-weight: 700; letter-spacing: .05em; text-transform: uppercase; border: 1px solid var(--border2); color: var(--text3); font-family: var(--font-ui); }
  ...
  /* Collapsible body — text + evidence chips. Hidden by default, expanded on row click. */
  ...
  .a-body {
    ...
    padding: 6px 0 4px calc(34px + 8px + 6px + 8px); /* align under title */
  ...
  /* Evidence chips — data traceability for AI signals */
  .a-evidence {
  ...
  /* ── CB RATES CLICKABLE ROWS ──────────────────────────────────── */
  #cbrates-tbody tr[data-cbr-id]:hover td {
    background: var(--bg3);
  }

  /* Correlations — clickable rows */
  ...
  /* ── PAIR DETAIL PANEL ───────────────────────────────────────── */
  /* #pair-detail removed v7.12.22 — replaced by #pd-popover floating overlay */
  ...
  /* Wide cell spans both columns — for retail positioning bar */
  ...
  /* ── EXPAND-IN-ROW (inline accordion) ───────────────────────── */
  /* The expansion row sits immediately below the selected pair row */
  ...
  /* Scroll wrapper fills the full td width; pd-inline is min-width:max-content
     so it never wraps — scrolls horizontally on narrow screens (split mode).
     On wide screens the wrapper is wider than pd-inline so we use
     justify-content:stretch via a flex trick on the wrapper. */
  .pd-inline-scroll {
    display:flex;
    overflow-x:auto;
    overflow-y:hidden;
  ...
  /* Price block left: pair, price, chg */
  .pd-inline-price {
  ...
  /* ── CARRY TRADE RANKING v2 (index2 / dashboard2.js) ─────────── */
  /* Shared grid definition — hdr and rows must always use identical columns so
     headers track content at every sidebar width (120px–320px).
     Column logic: PAIR fixed 46px · NOM. and CIP share remaining space equally ·
     SIGNAL gets enough for "▲ Positive" at 9px mono (~62px min) but grows with space.
     minmax() lets SIGNAL shrink gracefully without stealing space from NOM./CIP.     */
  .carry-rank-hdr,
  ...
  /* v2 row cells — font-size:10px matches Key Correlations td; cr-pair defined in v1 */
  ...
  /* NEWS FEED SCROLLBAR — custom thin dark */
  ...
  /* skew table 4-col — fixed layout prevents Direction column overflow in 220px panel */
  ...
  /* Limit the wide DATA item so it doesn't crowd the centered copyright */
  ...
  .footer-copy {
    ...
    pointer-events: none; /* sits above flex flow but never blocks clicks */
  }
  @media (max-width: 600px) {
    .footer-copy { display: none; }
  }

  /* Flag icons in sidebar tables */
  .fi { width: 16px; height: 12px; display: inline-block; }

  /* SENTIMENT PANEL — native render (no iframe) */
  ...
  @media (max-width: 900px) {
    /* Topbar: two-row layout — row1: logo + right controls, row2: nav tabs */
    #topbar {
      padding: 0 6px;
      height: auto;
      flex-wrap: wrap;
    ...
    /* Push topbar-right to far right of row 1 */
    .topbar-right {
      margin-left: auto;
      font-size: 10px;
      padding: 4px 0;
    ...
    /* Hide clock on mobile to save header space */
    #clock { display: none; }
    .live-dot { display: none; }
    /* Nav tabs: full-width second row, horizontally scrollable */
    .top-nav {
      display: flex;
      overflow-x: auto;
      overflow-y: hidden;
    ...
    /* Layout: stack sidebar → main → right panel vertically */
    #layout { display: flex; flex-direction: column; height: auto; overflow: visible; }

    /* Sidebar: horizontal scrollable strip at top */
    #sidebar {
    ...
    /* Main + Right: full width, auto height */
    #main { width: 100%; overflow-y: visible; overflow-x: hidden; }
    #rightpanel { width: 100%; border-left: none; border-top: 1px solid var(--border); }

    /* Narrative: stack label on top, full width text */
    #narrative { flex-direction: column; gap: 6px; }
    .narr-label { border-right: none; border-bottom: 1px solid var(--border2); padding-right: 0; padding-bottom: 4px; }
    .narr-text { font-size: 11px; width: 100%; }
    .narr-regime { align-self: flex-start; }

    /* 2-col grids that should become 1-col on mobile */
    ...
    /* Cross-asset: always keep 2 cols — compact enough for mobile */
    #cross-asset-grid { display: grid !important; grid-template-columns: 1fr 1fr !important; }

    /* Cross-asset: keep 2 cols (compact enough) */
    ...
    /* Cross-asset + Risk monitor panels: fixed height → auto */
    ...
    /* Indicator + Spread tables in main panel: no min-width, fit container width */
    table:has(#risk-indicators-tbody),
    table:has(#yield-spreads-tbody) { min-width: unset !important; width: 100% !important; }
    /* Fallback for browsers without :has() — target by container */
    #section-risk table,
    ...
    /* Sessions: add left breathing room to first column so it's not cut off */
    #section-crossasset td:first-child,
    #section-crossasset th:first-child { padding-left: 8px !important; }
    /* Sessions: add right breathing room to last column so it's not cut off */
    #section-crossasset td:last-child,
    #section-crossasset th:last-child { padding-right: 8px !important; }

    /* Market sentiment: no forced height */
    /* Myfxbook widget overflows its fixed internal table — clip + scroll horizontally */


    /* Reference Spreads: ensure it fills full width when stacked */
    #spreads-rows { width: 100%; box-sizing: border-box; }
    .spread-row { grid-template-columns: 48px 1fr 42px; gap: 4px; }

    /* Retail FX Positioning: fixed height + internal scroll on mobile */
    ...
    /* Indicator + Yield Spread tables: show all 3 columns on mobile.
       Label col truncates with ellipsis if too long; Value and Signal
       shrink to content and never wrap or get clipped. */
    ...
    /* Col 1 (label): fills remaining space, truncates with ellipsis */
    ...
    /* Col 2 (Value) and Col 3 (Signal): fixed narrow width, no wrapping */
    ...
    /* Compact font + padding for these tables on mobile */
    ...
    /* Footer: wrap cleanly */
    #statusbar { flex-wrap: wrap; gap: 4px; font-size: 9px; padding: 4px 8px; }
    #statusbar .footer-copy { position: static; transform: none; width: 100%; order: 10; text-align: center; }

    /* COT panel: fit all 8 cols within mobile viewport (overflow-x:hidden on #main prevents scroll) */
    .cot-scroll-wrap {
      overflow-x: visible;
    }
    .cot-header,
    .cot-row {
      grid-template-columns: 28px 60px 30px 48px 40px 10px 40px 52px;
      gap: 4px;
      min-width: 0;
    }
    ...
    /* Pair detail popover: bottom sheet on mobile */
    #pd-popover {
    ...
    /* Expand-in-row: keep horizontal layout, just remove the fixed height cap */
  ...
  @media (max-width: 480px) {
    ...
    .logo-sub { display: block; font-size: 7px !important; } /* keep "FX Terminal" visible */
    .topbar-right { font-size: 9px; gap: 6px !important; }
    /* Session label shortened */
    #session-label { max-width: 70px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .top-nav a { padding: 5px 10px; font-size: 10px; }
    /* Ensure main layout doesn't clip content below header */
    #layout { overflow: visible !important; }
    #main { overflow-x: hidden; overflow-y: visible !important; }

    /* Liquidity canvas: fill on mobile */
  ...
  /* invisible bridge so menu doesn't close when moving from button to panel */
  .site-menu::after {
...
/* ── Focus visible (WCAG 2.4.7) — restore outline stripped by reset ── */
:focus-visible {
...
/* ── CLS fix: skeleton placeholders for TradingView widgets ── */
/* Shown before the iframe loads — reserves the exact space to prevent layout shift */
.tv-skeleton {
  position: absolute;
  inset: 0;
  background: var(--bg2);
...
/* ── KEYBOARD HINT BUTTON (statusbar) ──────────────────────────────── */
.kb-hint-btn {
  font-size: 9px; font-family: var(--font-mono);
  background: transparent; border: 1px solid var(--border2);
...
.kb-hint-btn:hover { border-color: var(--blue); color: var(--blue); }

/* ── ETF OPTIONS IV PANEL ───────────────────────────────────────── */
.etf-iv-row {
  display: flex; align-items: center; gap: 5px;
  padding: 3px 8px; border-bottom: 1px solid var(--border);
  transition: background .1s;
...
/* ── PAIR DETAIL — TOOLTIPS (handled by #fx-tt engine in JS) ──────── */
.pd-cell { position: relative; }
.pd-cell.fx-tip { cursor: help; }

/* ── CONFIGURABLE ALERTS PANEL ──────────────────────────────────── */
#alerts-panel {
  border-bottom: 1px solid var(--border);
}
.alert-row {
...
.alert-add-btn:hover { border-color: var(--blue); color: var(--blue); }
.alert-del {
...
/* ── ALERTS POPOVER (statusbar bell icon) ───────────────────────── */
#alerts-popover {
  position: fixed;
  width: 280px;
...
/* ── LAYOUT RESIZE HANDLES (v7.71.0) ────────────────────────────────────
 * Drag handles inserted between grid columns by layout-resizer.js.
 * Only visible on desktop (> 900px) — hidden via display:none on mobile.
 * ─────────────────────────────────────────────────────────────────────── */
@media (min-width: 901px) {
  .gi-layout-handle {
    ...
    flex-shrink: 0;
    /* Expand hit area without affecting layout width */
    margin: 0 -3px;
    padding: 0 3px;
...
/* ── Economic Surprises panel — CESI-style bar chart (v7.77.0) ── */
#econ-surprise-table {
  table-layout: fixed;
}
...
/* Clickable Economic Surprises rows */
