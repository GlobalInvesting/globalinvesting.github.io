#!/usr/bin/env python3
"""
fetch_intraday_quotes.py  v3.13  —  Intraday quotes via yfinance + Twelve Data staleness fallback (+ FX pairs, CBOE/PHLX/CME/ETF IV cascade, correlations + signals.json normalization + context_snapshot enrichment)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Produce:  intraday-data/quotes.json
Schedule: Cada 15 min en días de semana, horario de mercado (via GitHub Action)

MEJORAS v3.13 vs v3.12:
  - **Root cause found for usdchf/usdcad's null IV — not a missing source, a
    single-strike selection bug.** Santiago linked Yahoo's live FXF options page
    (screenshot, 2026-08-06): spot=108.45, and the exact-ATM strike (K=108,
    |Δ|=0.45) had bid=0.00 (IV=27.10%, a stale-price artifact — the has-bid gate
    correctly rejects it), but the very next strike out (K=110, |Δ|=1.55, 1.4%
    from spot) had a real bid=0.10 and IV=11.18% — a plausible quote (1.7x
    usdchf's HV30 of 6.62%, well inside the 2.5x gate, in line with every other
    major). `_atm_iv_from_ticker()` only ever tried the single closest-to-spot
    strike and gave up entirely on no-bid — discarding a perfectly good nearby
    quote every time the exact round-number strike happened to be unquoted
    (common on thin FX ETF/index option chains — market makers frequently skip
    the exact ATM strike while quoting the ones either side of it).
  - **Fix:** `_atm_iv_from_ticker()` now tries up to `_IV_STRIKE_CANDIDATES=5`
    strikes nearest to spot (closest first, call then put at each), capped at
    `_IV_STRIKE_MAX_PCT=3%` of spot so a "near-ATM" fallback strike can't drift
    far enough to pick up meaningful vol skew instead of the ATM level. Stops at
    the first strike with a live bid. Verified offline against the exact
    screenshot data (108.45 spot, 7 strikes incl. the 108/110 pair above): new
    logic correctly skips K=108/K=109 (no bid) and resolves K=110 → IV=11.18%,
    1.69x HV30 — passes cleanly.
  - Output now flags when the resolved strike wasn't the true closest one:
    `source` becomes `"{label} options (near-ATM)"` instead of
    `"{label} options"`, and the console log adds a `(near-ATM, X.X% from spot —
    exact-ATM strike had no bid)` note — keeps the UI/log honest about which
    quotes are exact-ATM vs. nearby, without a schema change (still a plain
    string field).
  - This fix applies to the whole cascade (PHLX, CME, ETF — everywhere
    `_atm_iv_from_ticker()` is called), not just FXF/usdchf — same failure mode
    was plausible on FXC/usdcad and on the newly-added ^XDS/^XDC (v3.12) too.
    Doesn't guarantee usdchf/usdcad now resolve every run (still depends on live
    liquidity at whichever strike ends up with a bid), but removes a real,
    confirmed false-negative rather than papering over it.
  - `python3 -m py_compile` passes. Offline simulation (see above) confirms the
    selection logic against real screenshot data; full live confirmation still
    needs an actual GitHub Actions / local run since this environment has no
    network access to Yahoo.

MEJORAS v3.12 vs v3.11:
  - Investigated why usdchf/usdcad show null ATM IV on most/every run (Santiago
    flagged this after FIX-30/31c — those fixes made the null "honest" by
    rejecting fabricated-looking FXF/FXC quotes, they never added coverage).
    Probed whether the Nasdaq PHLX World Currency Options product line — already
    the source for AUD (^XDA) and NZD (^XDZ) — also lists CHF/CAD under ^XDS/^XDC,
    since that's a real exchange-listed index, unlike the never-existent "^AUDVIX".
    Ran a standalone probe (test_phlx_wco_chf_cad.py, not part of production)
    against live Yahoo data 2026-08-06: both tickers DO resolve (^XDS spot
    123.05, ^XDC spot 71.37) and DO carry a live options chain — but the nearest
    expiry's ATM contract on both has bid=0 (^XDS: K=135 vs spot 123.05 — a wide,
    thin strike grid; ^XDC: K=71 vs spot 71.37). Same "technically-live but
    economically stale/no-market" failure mode FIX-30 was built to catch on FXF,
    not a new problem — the has-bid gate in _atm_iv_from_ticker() already handles
    it correctly if these tickers are wired in.
  - Added `"usdchf": "^XDS"` and `"usdcad": "^XDC"` to _PHLX_WCO_INDEX. This does
    NOT resolve the current gap (confirmed illiquid at probe time) — added per
    the project's "keep dead-but-real sources as harmless fast-failing
    fallbacks" principle (same as CME 6A=F for AUD): it's a genuine exchange
    product, costs one extra fast-failing API call per run when unused, and
    self-heals with zero further code changes if PHLX liquidity in these two
    contracts ever improves. Positioned as Source 0.5 (before CME/ETF), same
    slot as AUD/NZD, since it's the same product line.
  - **Conclusion, documented for future sessions:** as of 2026-08-06, usdchf and
    usdcad have no free public source that reliably clears the liquidity/HV
    plausibility gates — CBOE/CME vol index doesn't cover them, CME futures
    options don't resolve via yfinance, PHLX WCO exists but is illiquid at the
    ATM strike, and the ETF options (FXF/FXC) are the same "frequently
    illiquid" story already documented. Same category of exhausted-free-sources
    conclusion as the AUD OIS-implied forward search — nothing else left to try
    without a paid feed (CME CVOL, still DataMine/CME-Direct-only per the
    existing FIX-31b note).
  - Runtime banner in main() was hardcoded at "v3.8" while the docstring header
    had already moved to v3.11 — the exact drift pattern GUIDELINES.md already
    documents as an incident (2026-08-02, fetch_bond_yields.py / this file).
    Fixed while touching this file; both strings now read v3.12.

MEJORAS v3.11 vs v3.10:
  - FIX-32: "BRENT/USD" was never a valid Twelve Data symbol — TD's own product
    list (twelvedata.com/forex, twelvedata.com/commodities) shows Brent's ticker
    code is "XBR", not "BRENT". Verified against TD's live market page
    (twelvedata.com/markets/874865/commodity/xbr-usd). Every batch call that
    included brent was returning "symbol not found" for that leg specifically —
    silver (XAG/USD) and WTI (WTI/USD) already use TD's correct documented codes
    and were not touched.
  - The v3.5 comment claiming XAU/XAG/WTI/BRENT were "confirmed accessible on
    TD free tier" was never actually verified against a live call (no network
    access to api.twelvedata.com from this dev environment either) — flagged as
    unverified per data-integrity rule (no stating something is confirmed
    without a real source check). Root cause above (bad symbol) explains
    brent's failure with certainty; silver/WTI failures reported in Santiago's
    logs are NOT explained by a symbol error (their TD codes were already
    correct) and remain unconfirmed — most likely a free-tier plan restriction
    gating individual commodity symbols (TD gates some symbols per-instrument,
    same pattern already seen with the equity indices removed in v3.6).
  - fetch_td_batch() error logging now surfaces TD's numeric `code` field
    (e.g. 403 = plan-restricted, 404 = symbol not found) alongside the message,
    instead of just the message string — so the next live run tells us
    definitively whether silver/WTI are a plan-tier gate (in which case no code
    fix can resolve it — would need a paid TD tier or a different fallback
    source) or something else neither of us has seen yet.

MEJORAS v3.10 vs v3.9:
  - FIX-31a: "^AUDVIX" en _CBOE_FX_VOL_INDEX era un ticker inexistente — CBOE/CME
    nunca publicaron un índice de volatilidad para AUD bajo ningún símbolo (el
    listado oficial de CBOE solo documenta tres: EUVIX, JYVIX, BPVIX). Confirmado
    en el log en vivo (404 "Quote not found for symbol: ^AUDVIX" en cada corrida)
    y por fuentes externas. Efecto en cascada: audusd caía por descarte a
    Sources 1-3 en cada corrida (siempre fallidas — CME 6A=F/6AU26/6AZ26 sin
    datos vía yfinance), y como nzdusd dependía enteramente de audusd (proxy
    ×1.08), ambos quedaban sin IV la mayoría de las corridas — visible en el
    Volatility Leaderboard (solo combinaciones EUR/GBP/JPY en el top 5) y en
    cualquier cross con pata AUD/CHF/CAD/NZD mostrando "—" en el panel de
    detalle (reportado por Santiago sobre EUR/CHF). Eliminado "^AUDVIX" del
    diccionario; ya no se intenta ese ticker.
  - FIX-31b: nueva Source 0.5 — PHLX World Currency Options index (^XDA para
    AUD, ^XDZ para NZD). Producto real de Nasdaq (desde 1982, "United Currency
    Options Market"), listado activamente hoy; a diferencia de ^AUDVIX, estos
    tickers sí resuelven en Yahoo y llevan una cadena de opciones real. NZD ya
    no depende exclusivamente del proxy ×1.08 de AUD — ahora intenta su propia
    fuente (^XDZ) primero, y solo cae al proxy (relabeled "est. AUD × 1.08",
    ya no "est. CBOE ^AUDVIX × 1.08") si ^XDZ también falla. Evaluados también
    los CVOL Index de CME (ADVL/CAVL/CHVL, cubren AUD/CAD/CHF) como alternativa:
    descartados porque solo se distribuyen vía CME Direct/DataMine con
    suscripción paga — confirmado fetcheando la página pública de CME, que no
    expone el valor numérico en el HTML, solo un link promocional al producto
    licenciado.
  - FIX-31c: nuevo gate de plausibilidad relativo a HV30 (_IV_HV_RATIO_MAX=2.5)
    en _atm_iv_from_ticker(), aplicado a las fuentes 0.5-3 (no a la Source 0 —
    un índice institucional puede divergir legítimamente de la vol realizada).
    Root-caused 2026-08-06 sobre una corrida real: usdchf vía FXF resolvió con
    bid activo (pasando el gate de liquidez del FIX-30) en IV=22.9%, pero su
    propia HV30 era 6.24% — ratio 3.7x, cuando todo el resto de majors ronda
    1.1-2.0x su propia HV30. El gate de "hay bid" no alcanza a filtrar una
    cotización técnicamente viva pero económicamente stale/angosta. Visible en
    producción: EUR/CHF triangulado mostró ATM IV 19.5%, IV−HV +16.3pp (vs.
    1-3pp en el resto de pares). Mismo día que se corrigió usdcad vía FXC:
    IV=11.0% vs HV30=3.47% (ratio 3.17x) — también hubiera pasado sin este fix.
    Cuando el gate rechaza, la cascada cae al siguiente candidato en vez de
    aceptar el número; si no queda ninguno, el par queda honestamente sin IV
    ("—" en la UI) en vez de mostrar un valor fabricado — mismo principio de
    integridad de datos que el FIX-30.
  - Actualizados los tooltips de dashboard.js (ATM IV inline, panel de detalle
    completo, tabla de Positioning Bias) y about.html (sección "Estimated
    data") para reflejar la nueva cascada: ya no mencionan "^AUDVIX" en ningún
    lado; documentan PHLX ^XDA/^XDZ como fuente de AUD/NZD.
  - Verificado con test de integración simulado (mock de yf.Ticker, sin acceso
    de red real): los 7 pares resuelven en el orden de cascada correcto, el
    gate HV-relativo rechaza los dos casos reales documentados arriba y acepta
    lecturas plausibles, y el fallback NZD→AUD×1.08 funciona cuando ^XDZ falla.
    dashboard.test.js: 90/90 passing (sin cambios de lógica en dashboard.js,
    solo texto de tooltips).

MEJORAS v3.9 vs v3.8:
  - FIX-30: _atm_iv_from_ticker() no filtraba por liquidez del quote en el
    strike ATM — solo exigía impliedVolatility > 0. Un contrato con bid=0
    (sin mercado activo) puede devolver un impliedVolatility calculado sobre
    un lastPrice obsoleto, que igual pasaba el gate de plausibilidad
    [_IV_MIN,_IV_MAX]=[3,40]%. Root-caused 2026-08-06 vía el nuevo panel
    Volatility Leaderboard: usdchf cayó en cascada hasta el último recurso
    (opciones del ETF FXF — ya documentado en el código como "frequently
    illiquid") y devolvió IV=29.4% vs. 7-12% del resto de majors, contaminando
    los 4 crosses de CHF triangulados en dashboard.js. Ahora exige bid>0 (si
    la columna existe) en el ATM call antes de aceptar su IV; si falla, intenta
    el ATM put con el mismo filtro; si ambos fallan, la función devuelve None
    y la cascada de fuentes sigue con el siguiente candidato en vez de aceptar
    un quote sin mercado — consistente con el principio de "no fabricar datos"
    del proyecto.

MEJORAS v3.8 vs v3.7:
  - FIX-29: PASO 4 (reemplazo us2y con FRED:DGS2) ya no recicla para siempre el
    prev_close/chg que hubiera en quotes.json (ni siquiera venía de un 2Y real —
    heredaba del proxy ^IRX de PASO 1-3, un 3M). Ahora lee bond2y_prev desde
    extended-data/USD.json (escrito por fetch_bond_yields.py v2.8 FIX-29, misma
    corrida de FRED:DGS2) y calcula chg/pct a partir de ese par real. Si
    bond2y_prev no está disponible en una corrida puntual, se cae a mantener el
    valor existente en vez de fabricar uno — no rompe el pipeline si el fix del
    lado de fetch_bond_yields.py todavía no se desplegó.
...
# ── FX Implied Volatility — CBOE/CME FX Vol Indexes (primary) + PHLX WCO + CME options + ETF ─
#
# SOURCE CASCADE — priority order per pair:
#
#   SOURCE 0 — CBOE/CME FX Volatility Indexes (^EUVIX, ^BPVIX, ^JYVIX)
...
#     Coverage: EUR, GBP, JPY only. CBOE/CME never published an AUD index under any
#     ticker — "^AUDVIX" was a fabricated/nonexistent symbol (removed 2026-08-06;
#     confirmed via Yahoo 404 "Quote not found" and via CBOE's own product list, which
#     documents exactly three FX vol indexes: EUVIX, JYVIX, BPVIX). CME does publish a
#     real AUD/CAD/CHF volatility benchmark (CVOL — ADVL/CAVL/CHVL), but it is only
#     distributed via CME Direct / DataMine under a paid license, not a free public feed,
...
#
#   SOURCE 0.5 — PHLX World Currency Options index (^XDA, ^XDZ, ^XDS, ^XDC)
#     AUD, NZD, CHF, CAD. Nasdaq PHLX has traded USD-settled currency options on these
#     since 1982 ("United Currency Options Market") and the product is still listed
#     today. The index ticker itself carries a live option chain on Yahoo (unlike
#     ^AUDVIX, these actually resolve). Same option-chain extraction and liquidity/
#     plausibility gates as sources 1-3 below — this is not a clean index value like
#     Source 0, so it is gated identically to the CME/ETF fallbacks.
#     CHF/CAD note (probed 2026-08-06, test_phlx_wco_chf_cad.py): ^XDS and ^XDC both
#     resolve with a live options chain, but the nearest-expiry ATM contract carries
#     bid=0 on both — confirmed illiquid, not confirmed useless. Kept in the cascade
#     as a harmless fast-failing candidate (same rationale as CME 6A=F for AUD);
#     as of this date it does not actually resolve for either pair.
#
...
#
#   SOURCE 3 — CBOE ETF options (FXE, FXB, FXY, FXA, FXF, FXC)
#     Last resort. Retail-oriented; yfinance frequently returns impliedVolatility=0.
#     Retained only for USD/CHF and USD/CAD where no CBOE vol index exists, and as the
#     final fallback for EUR/GBP/JPY/AUD when their primary sources are unavailable.
#
# NZD coverage:
#   No CBOE/CME vol index and no CME futures-options/ETF ticker exists for NZD/USD.
#   Source 0.5 (^XDZ) is tried first; if that also fails, fall back to deriving from
#   AUD/USD's *resolved* IV (whatever source produced it) × 1.08 (long-run NZD/AUD
#   realised vol ratio), labeled "est. AUD × 1.08".
#
...
#
# IV plausibility gates (option-chain-derived sources only — 0.5, 1, 2, 3):
#   Absolute: 3.0%–40.0% (covers normal FX vol range 4–20% plus stress)
#   HV-relative: IV must not exceed _IV_HV_RATIO_MAX × the pair's own HV30. Root-caused
#   2026-08-06: a live-but-thin FXF (usdchf) bid passed the absolute gate and the
#   has-bid liquidity gate at IV=22.9% while usdchf's own HV30 was 6.24% (3.7x) — every
#   genuine major sits at 1.1-2.0x its own HV30. The has-bid check alone doesn't catch a
#   technically-live but economically stale/wide single-sided quote; this gate does.
#   Source 0 (clean CBOE index values) is intentionally NOT subject to the HV-relative
#   gate — it's a forward-looking institutional benchmark that can legitimately diverge
#   from realised vol around events, and second-guessing it against our own HV would
#   defeat its purpose.
...
_IV_HV_RATIO_MAX = 2.5  # option-chain-derived IV rejected if it exceeds this × the pair's own HV30
_IV_STRIKE_CANDIDATES = 5   # how many nearest-to-spot strikes to try before giving up
_IV_STRIKE_MAX_PCT = 0.03  # tolerance band: only strikes within 3% of spot count as "near-ATM"
...
# Coverage limited to 3 major pairs; AUD/CHF/CAD/NZD fall through to PHLX/CME/ETF.
_CBOE_FX_VOL_INDEX = {
    "eurusd": "^EUVIX",
    "gbpusd": "^BPVIX",
    "usdjpy": "^JYVIX",
    # AUD: no CBOE/CME index exists (see cascade docstring above) — falls to
    #   Source 0.5 (^XDA), then CME 6A options/IMM/FXA ETF.
    # CHF: no CBOE index — falls to Source 0.5 (^XDS, confirmed illiquid as of
    #   2026-08-06 — see cascade docstring), then CME 6S options / FXF ETF.
    # CAD: no CBOE index — falls to Source 0.5 (^XDC, confirmed illiquid as of
    #   2026-08-06 — see cascade docstring), then CME 6C options / FXC ETF.
    # NZD: no CBOE index — falls to Source 0.5 (^XDZ), then AUD × 1.08 proxy.
}

# PHLX World Currency Options index tickers. Real, currently-listed Nasdaq product
# (since 1982); the index itself carries an options chain on Yahoo, unlike the
# nonexistent ^AUDVIX. ^XDA/^XDZ (AUD/NZD) resolve live. ^XDS/^XDC (CHF/CAD) also
# resolve as tickers with a real chain but were found illiquid (bid=0 at ATM) when
# probed 2026-08-06 — kept as a fast-failing candidate rather than removed, per
# project convention for genuine-but-currently-dead sources.
_PHLX_WCO_INDEX = {
    "audusd": "^XDA",
    "nzdusd": "^XDZ",
    "usdchf": "^XDS",
...
def _atm_iv_from_ticker(sym, label, hv30=None):
    """
    Extract ATM implied volatility from a yfinance option chain.
    Returns dict { iv, expiry, atm, source } or None.
    IV plausibility gates: absolute _IV_MIN–_IV_MAX %, and — when hv30 is supplied —
    IV must not exceed _IV_HV_RATIO_MAX × hv30 (see cascade docstring above).
    """
    ...
    try:
        ...
        # ── Near-the-money strike selection, with liquidity fallback across nearby strikes ──
        # Root-caused 2026-08-06 against live FXF options data (Yahoo UI, Santiago's
        # screenshot): spot=108.45, the single EXACT-ATM strike (K=108, |Δ|=0.45) had
        # bid=0.00 — no market, IV=27.10% is a stale-price artifact — while the next
        # strike out (K=110, |Δ|=1.55, 1.4% from spot) had a real bid=0.10 and
        # IV=11.18%, a plausible quote (~1.7x usdchf's HV30 of 6.62%, in line with
        # every other major). Market makers frequently skip quoting the exact
        # round-number strike closest to spot on thin FX ETF/index option chains,
        # while a strike one tick over carries a live two-sided market. The old logic
        # tried only the single closest strike and gave up entirely if it had no bid
        # — discarding a perfectly good nearby quote. Now tries up to
        # _IV_STRIKE_CANDIDATES strikes, closest-to-spot first, capped at
        # _IV_STRIKE_MAX_PCT of spot (skew risk grows with distance from spot — the
        # cap keeps "near-ATM" from drifting into a strike whose IV reflects the vol
        # skew rather than the ATM level).
        strikes_sorted = sorted(calls["strike"].dropna().tolist(), key=lambda s: abs(s - spot))
        if not strikes_sorted:
            return None
        max_dist = spot * _IV_STRIKE_MAX_PCT
        candidate_strikes = [s for s in strikes_sorted if abs(s - spot) <= max_dist][:_IV_STRIKE_CANDIDATES]
        if not candidate_strikes:
            # Nothing within tolerance — fall back to the single closest strike so
            # the has-bid gate below still gets a fair shot (matches old behaviour
            # for chains with a coarse strike ladder, e.g. far-apart index strikes).
            candidate_strikes = strikes_sorted[:1]

        # Extract IV: try each candidate strike's call first, then its put, in
        # closest-to-spot order. Liquidity gate: require an active bid before
        # trusting impliedVolatility — a 0-bid quote still passes the [3,40]%
        # plausibility gate below, so it must be rejected here instead. Thin
        # ETF-options fallbacks (FXF, etc. — documented above as "frequently
        # illiquid") can carry a stale/zero-bid quote whose IV field is a
        # stale-price artifact, not a market-priced read. Root-caused 2026-08-06:
        # usdchf via FXF's exact-ATM strike returned IV=29.4% this way (vs. 7-12%
        # for all other majors).
        iv_raw = None
        atm_strike = None
        puts = chain.puts
        for k in candidate_strikes:
            atm_calls = calls[calls["strike"] == k]
            if not atm_calls.empty:
                row = atm_calls.iloc[0]
                v = row["impliedVolatility"]
                has_bid = "bid" not in row or bool(row["bid"] and row["bid"] == row["bid"] and row["bid"] > 0)
                if v and v == v and v > 0 and has_bid:  # truthy + not NaN + positive + live bid
                    iv_raw, atm_strike = v, k
                    break

            atm_puts = puts[puts["strike"] == k]
            if not atm_puts.empty:
                row = atm_puts.iloc[0]
                v = row["impliedVolatility"]
                has_bid = "bid" not in row or bool(row["bid"] and row["bid"] == row["bid"] and row["bid"] > 0)
                if v and v == v and v > 0 and has_bid:
                    iv_raw, atm_strike = v, k
                    break

        if iv_raw is None:
            print(f"    [{label}] IV null/no-bid at all {len(candidate_strikes)} near-spot strikes "
                  f"tried ({candidate_strikes})  spot={spot:.5f}")
            return None
        ...
        # HV-relative plausibility gate — see cascade docstring above for the
        # 2026-08-06 FXF/usdchf root-cause (22.9% IV vs. 6.24% HV30 = 3.7x, passed
        # the absolute gate and the has-bid check but was still a stale/thin quote).
        if hv30 and hv30 > 0:
            ratio = iv_pct / hv30
            if ratio > _IV_HV_RATIO_MAX:
                print(f"    [{label}] IV {iv_pct:.1f}% is {ratio:.1f}x HV30 ({hv30:.1f}%) "
                      f"— rejecting as implausible (max {_IV_HV_RATIO_MAX}x)")
                return None

        days_left = (_iv_date.fromisoformat(chosen_exp) - today).days
        moneyness_pct = abs(atm_strike - spot) / spot * 100
        near_note = "" if atm_strike == strikes_sorted[0] else f"  (near-ATM, {moneyness_pct:.1f}% from spot — exact-ATM strike had no bid)"
        print(f"    [{label}] ✓ IV={iv_pct:.1f}%  spot={spot:.5f}  K={atm_strike}  exp={chosen_exp} ({days_left}d)  n={len(calls)}{near_note}")
        return {
            "iv":     iv_pct,
            "expiry": chosen_exp,
            "atm":    round(atm_strike, 6),
            "source": f"{label} options" if atm_strike == strikes_sorted[0] else f"{label} options (near-ATM)",
        }
...
_FX_IV_MAP = {
    ...
    # NZD: no CME futures-options or ETF ticker exists. Tries Source 0.5 (^XDZ)
    # first, then falls back to the AUD × 1.08 proxy in fetch_fx_etf_iv().
    "nzdusd": {"cme": None,  "etf": None,  "invert": False},
}


def fetch_fx_etf_iv(hv30_map=None):
    """
    Fetches ATM implied volatility for 7 major FX pairs.

    Source cascade per pair (see module docstring above for full rationale):
      0.   CBOE/CME FX Volatility Indexes (^EUVIX, ^BPVIX, ^JYVIX) — EUR/GBP/JPY only.
           Institutional benchmark — same variance-swap methodology as VIX.
           Unconditional priority when available; not subject to the HV-relative gate.
      0.5. PHLX World Currency Options index (^XDA, ^XDZ, ^XDS, ^XDC) — AUD/NZD/
           CHF/CAD. ^XDS/^XDC (CHF/CAD) confirmed illiquid at probe time
           (2026-08-06) — kept as a fast-failing candidate, see module docstring.
    ...
    Sources 0.5-3 all pass through the same absolute [_IV_MIN,_IV_MAX]% gate plus the
    HV-relative gate (IV ≤ _IV_HV_RATIO_MAX × the pair's own HV30, when hv30_map is
    supplied) inside _atm_iv_from_ticker().

    NZD/USD: tries Source 0.5 (^XDZ) first. If that also fails, falls back to deriving
    from AUD/USD's resolved IV (whatever source produced it) × 1.08 (long-run NZD/AUD
    realised vol ratio), labeled "est. AUD × 1.08".

    Args:
        hv30_map: optional dict { pair_id: {"hv30": float|None, ...} }, e.g. the return
            value of fetch_hv30_fx(). Used only for the HV-relative plausibility gate;
            pairs missing from the map simply skip that gate (absolute gate still applies).

    Returns dict { pair_id: { "iv": float_pct, "source": str } | None }.
    """
    print("\n[FX-IV] Fetching ATM implied volatility (CBOE vol index → PHLX WCO → CME options → ETF)...")
    hv30_map = hv30_map or {}
    results = {}
    ...
    for pair_id, cfg in _FX_IV_MAP.items():
        cme_root = cfg["cme"]
        etf_sym  = cfg["etf"]
        pair_hv30 = (hv30_map.get(pair_id) or {}).get("hv30")

        # ── SOURCE 0: CBOE/CME FX Volatility Index (EUR/GBP/JPY only) ──
        ...
        # ── SOURCE 0.5: PHLX World Currency Options index (AUD/NZD only) ──
        phlx_sym = _PHLX_WCO_INDEX.get(pair_id)
        result = None
        if phlx_sym:
            print(f"  [{pair_id}] source 0.5: PHLX World Currency Options ({phlx_sym})")
            result = _atm_iv_from_ticker(phlx_sym, f"PHLX {phlx_sym}", hv30=pair_hv30)
            if result:
                results[pair_id] = result
                print(f"  [FX-IV] ✓ {pair_id:8s}: IV={result['iv']:.1f}%  source={result['source']}  [PHLX]")
                continue

        # ── NZD/USD fallback: derive from AUD/USD's resolved IV when ^XDZ also fails ──
        if pair_id == "nzdusd":
            aud_result = results.get("audusd")
            if aud_result and aud_result.get("iv") is not None:
                nzd_iv = round(aud_result["iv"] * 1.08, 1)
                results[pair_id] = {"iv": nzd_iv, "source": f"est. AUD ({aud_result['source']}) × 1.08"}
                print(f"  [FX-IV] ~ {pair_id:8s}: IV={nzd_iv:.1f}%  [derived from AUD/USD]")
            else:
                results[pair_id] = None
                print(f"  [FX-IV] — {pair_id:8s}: no IV (AUD/USD also unavailable)")
            continue
        ...
        for sym, label in candidates:
            result = _atm_iv_from_ticker(sym, label, hv30=pair_hv30)
            if result:
                break
    ...
    print(f"[FX-IV] {valid}/{len(_FX_IV_MAP)} pairs resolved  "
          f"({cboe_index_resolved} via CBOE index, {valid-cboe_index_resolved} via PHLX/CME/ETF options)")
    return results
...
TD_BATCH_MAP = {
    # Commodities only. NOTE (v3.11): "confirmed accessible on TD free tier" below
    # was never actually verified live — gold has resolved successfully in past
    # runs, but silver/WTI have been reported failing with no confirmed root
    # cause (their TD symbol strings are correct, unlike brent's — see FIX-32
    # above). If TD's `code` field on the next run shows 403 for those, it's a
    # plan-tier gate and no symbol fix will help; watch the logs.
    ...
    "silver": "XAG/USD",
    "wti":    "WTI/USD",
    "brent":  "XBR/USD",  # FIX-32 (v3.11): was "BRENT/USD" — not a real TD symbol.
                           # TD's own ticker code for Brent is "XBR" (see
                           # twelvedata.com/markets/874865/commodity/xbr-usd).
}
    ...
    for td_sym, data in raw_items.items():
        ...
        if not isinstance(data, dict) or data.get("status") == "error":
            if isinstance(data, dict):
                # FIX-32 (v3.11): surface TD's numeric `code` too, not just the message —
                # 403 means plan-restricted (no code fix possible, needs a paid tier or a
                # different source), 404 means the symbol string itself is wrong (fixable
                # here). Without the code both failure modes printed identically and got
                # conflated (see brent's real 404 vs silver/WTI's still-unconfirmed cause).
                code = data.get("code", "?")
                msg  = data.get("message", "error")
                print(f"[TwelveData] ✗ {internal} ({td_sym}): [{code}] {msg}")
            else:
                print(f"[TwelveData] ✗ {internal} ({td_sym}): bad response")
            continue
...
def main():
    ...
    print(f"\n{'='*60}\nfetch_intraday_quotes.py  v3.13  —  {ts}\n{'='*60}\n")
    ...
    # PASO 4 (FIX W-05, corregido FIX-29 2026-08-02): Reemplazar us2y con el Treasury
    # 2Y real desde FRED:DGS2. ^IRX es el T-Bill de 13 semanas (3M), no el Treasury 2Y.
    # extended-data/USD.json contiene bond2y desde FRED:DGS2 (daily), que es el dato correcto.
    #
    # FIX-29: la version anterior hacía `existing_us2y.get("prev_close", ...)`, es decir,
    # reciclaba para siempre el prev_close/chg que ya hubiera en quotes.json (que ni
    # siquiera venía de un 2Y real — venía del ^IRX de PASO 1-3, un 3M). Una vez que
    # bond2y quedaba fijo, chg quedaba congelado en ese valor para siempre porque nunca
    # había una fuente nueva para "ayer". Ahora se usa bond2y_prev (escrito por
    # fetch_bond_yields.py FIX-29, misma corrida de FRED:DGS2) como prior close
    # número real. Ahora se usa bond2y_prev (escrito por fetch_bond_yields.py FIX-29,
    # misma corrida de FRED:DGS2) como prior close real, y chg/pct se calculan a
    # partir de ese par (bond2y, bond2y_prev) en vez de heredarse.
    # Si bond2y_prev no está disponible todavía (build previo a este fix, o fuente caída),
    # se cae a mantener el chg/prev_close existentes en vez de inventar uno.
    try:
        ...
        bond2y_prev      = usd_ext.get("data", {}).get("bond2y_prev")
        bond2y_prev_date = usd_ext.get("dates", {}).get("bond2y_prev", "")
        if bond2y and isinstance(bond2y, (int, float)) and 0 < bond2y < 20:
            existing_us2y = quotes.get("us2y", {})
            if bond2y_prev and isinstance(bond2y_prev, (int, float)) and 0 < bond2y_prev < 20:
                prev_close = round(float(bond2y_prev), 4)
                chg        = round(float(bond2y) - float(bond2y_prev), 4)
                pct        = round((chg / float(bond2y_prev)) * 100, 4) if bond2y_prev else 0.0
            else:
                # Fallback: no real bond2y_prev this run — keep whatever was already
                # there instead of fabricating a number (matches pre-FIX-29 fallback
                # behavior, but only as a last resort, not the default path).
            ...
            quotes["us2y"] = {
                ...
                "prevSourceDate": bond2y_prev_date or None,
            }
            print(f"[us2y] FRED:DGS2 = {bond2y:.4f}% ({bond2y_date}), prev {prev_close:.4f}% ({bond2y_prev_date or 'n/a'}) — reemplaza proxy ^IRX")
        else:
    ...
    # PASO 7: ATM implied volatility — CBOE index (primary) + PHLX WCO + CME options + ETF (fallback)
    fx_etf_iv = fetch_fx_etf_iv(hv30_map=hv30_data)
